...
(function () {
  ...
  .mer-pwa-modal .icons{display:flex;justify-content:center;gap:18px;margin:8px 0 6px;
    color:#5a4f78;font-size:13px}
  .mer-pwa-modal .icons div{display:flex;flex-direction:column;align-items:center;gap:4px}
  .mer-pwa-modal .icons div span{font-size:28px}
  ...
  fab.onclick = e => {
    if (e.target.classList.contains('x')) { // 关闭按钮
      localStorage.setItem(DISMISS_KEY, 'gone');
      fab.remove(); return;
    }
    if (IS_IOS || (!deferred && !IS_ANDROID)) {
      // 弹图解
      openModal();
    } else if (deferred) {
      // 原生气泡
